...
def escrapeo(url_escrapeo,ruta_service,ruta_fichero_escrapeo):
    """
    ...
    Parámetros:
        url_escrapeo(str): La URL de la página 
        ruta_service (str): La ruta del ejecutable de ChromeDriver necesaria para Selenium.
        ruta_fichero_escrapeo (str): La ruta donde se guardará el archivo pickle con los datos extraídos.
    ...
    service = Service(ruta_service)  #  path del chromedriver
    ...
    try:
        ...
        escrapeo(url_escrapeo,ruta_service,ruta_fichero_escrapeo)
    df=pd.DataFrame(dictio_scrap)
    df.to_pickle(ruta_fichero_escrapeo)
    driver.close()
    driver.quit()
    return df
